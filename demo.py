...
daisy_track = atlast_obj.scanning_pattern(start_time="2022-08-01T23:00:00",
    scan_pattern="daisy",
    scan_options={'radius': 0.8, 'speed': 1.0}, # in degrees
...
# load the input map 
input_map = atlast_obj.load_input_map(resolution=1.00E+00/3.60E+03,center=(260,-20),nu=93e9,units="Jy/pixel")
...
input_map, output_map, outhdu = atlast_obj.generate_output_map(center=(260,-20),units="uK_RJ",width=0.6,height=0.6,resolution=3.00E+00/3.60E+03,tod_preprocessing={"window": {"name": "tukey", "kwargs": {"alpha": 0.1}},"remove_modes": {"modes_to_remove": [0]},"remove_spline": {"knot_spacing": 10}})
...
beam_psd, spatial_freq, fwhm_rad, sigma_beam_dimensionless=atlast_obj.detector_beam_PS(max_radius, instrument, atlast_obj.inpsamp, outhdu)

## uncomment the following line to save the beam data
# pickle.dump({'tf':transfer_function, 'beam':beam_psd, 'spatial_freq':spatial_freq}, open("beam_data.pkl", "wb"))

## plot or save
atlast_obj.beam_ps_plot(beam_psd=beam_psd, spatial_freq=spatial_freq, transfer_function=transfer_function,scale=('log','linear'), save=True)
atlast_obj.transfer_function_avg_plot(beam_psd=beam_psd, spatial_freq=spatial_freq, transfer_function=transfer_function,scale=('log','linear'),save=True)

# save the log file ( captures all the events with date and time)
atlast_obj.save_log_file("demo_log_5.txt")   